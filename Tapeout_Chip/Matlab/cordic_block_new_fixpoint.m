function [RR] = cordic_block_new_fixpoint(RR,recursive_times,constant,Mode)
...
kapa= 0.609375;%(1+tan^2theta)^(-1/2)=costheta %fix(1/prod(sqrt(1+2.^(-2*(0:recursive_times-1)))) * constant)
...
%
    ...
    if(Mode == 1)
        for column_element = 2:M
            ...
            [upper_tri1] = cordic_vectoring_fixpoint(micro_rr,recursive_times,kapa,constant);%cord#1
