// VM2x8_micro_rotation.sv
`include "./parameter.sv"


module VM2x8_micro_rotation(
//A c 
//B d 
	input signed_logic A_in[7:0],    //第一列
	input signed_logic B_in[7:0],    //第二列
	...
	//最後角度是{p0,p1,p2,.....,p6},p0象限在外面做,所以p1最左邊
...
//-----------------------micro_1-----------2^0
	signed_logic A_1q[7:0], B_1q[7:0];
	signed_logic A_rm0to1[7:0], B_rm0to1[7:0], A_1d[7:0], B_1d[7:0];
	logic p_1;
	...
	genvar i_1;
	generate 
	for (i_1=0; i_1<8; i_1=i_1+1) begin:rm0to1
...
//暫存器(在每一層的micro_rotation後都要有一個reg，有load才會加載)
	genvar j_1;
	generate 
	for (j_1=0; j_1<8; j_1=j_1+1) begin:DFF_1
...
//-----------------------micro_2-----------2^1
	signed_logic A_2q[7:0], B_2q[7:0];
	signed_logic A_rm1to2[7:0], B_rm1to2[7:0], A_2d[7:0], B_2d[7:0];
	logic p_2;
	...
	genvar i_2;
	generate 
	for (i_2=0; i_2<8; i_2=i_2+1) begin:rm1to2
...
//暫存器(在每一層的micro_rotation後都要有一個reg，有load才會加載)
	genvar j_2;
	generate 
	for (j_2=0; j_2<8; j_2=j_2+1) begin:DFF_2
		always_ff@(posedge clk)begin
			if(rst)begin
				A_2q[j_2] <= 'b0;
				B_2q[j_2] <= 'b0;
...
//-----------------------micro_3-----------2^2
	signed_logic A_3q[7:0], B_3q[7:0];
	signed_logic A_rm2to3[7:0], B_rm2to3[7:0], A_3d[7:0], B_3d[7:0];
	logic p_3;
	...
	genvar i_3;
	generate 
	for (i_3=0; i_3<8; i_3=i_3+1) begin:rm2to3
...
//暫存器(在每一層的micro_rotation後都要有一個reg，有load才會加載)
	genvar j_3;
	generate 
	for (j_3=0; j_3<8; j_3=j_3+1) begin:DFF_3
		always_ff@(posedge clk)begin
			if(rst)begin
				A_3q[j_3] <= 'b0;
				B_3q[j_3] <= 'b0;
...
//-----------------------micro_4-----------2^3
	signed_logic A_4q[7:0], B_4q[7:0];
	signed_logic A_rm3to4[7:0], B_rm3to4[7:0], A_4d[7:0], B_4d[7:0];
	logic p_4;
	...
	genvar i_4;
	generate 
	for (i_4=0; i_4<8; i_4=i_4+1) begin:rm3to4
...
//暫存器(在每一層的micro_rotation後都要有一個reg，有load才會加載)
	genvar j_4;
	generate 
	for (j_4=0; j_4<8; j_4=j_4+1) begin:DFF_4
		always_ff@(posedge clk)begin
			if(rst)begin
				A_4q[j_4] <= 'b0;
				B_4q[j_4] <= 'b0;
...
//-----------------------micro_5-----------2^4
	signed_logic A_5q[7:0], B_5q[7:0];
	signed_logic A_rm4to5[7:0], B_rm4to5[7:0], A_5d[7:0], B_5d[7:0];
	logic p_5;
	...
	genvar i_5;
	generate 
	for (i_5=0; i_5<8; i_5=i_5+1) begin:rm4to5
...
//暫存器(在每一層的micro_rotation後都要有一個reg，有load才會加載)
	genvar j_5;
	generate 
	for (j_5=0; j_5<8; j_5=j_5+1) begin:DFF_5
		always_ff@(posedge clk)begin
			if(rst)begin
				A_5q[j_5] <= 'b0;
				B_5q[j_5] <= 'b0;
...
//-----------------------micro_6-----------2^5
	signed_logic A_6q[7:0], B_6q[7:0];
	signed_logic A_rm5to6[7:0], B_rm5to6[7:0], A_6d[7:0], B_6d[7:0];
	logic p_6;
	...
	genvar i_6;
	generate 
	for (i_6=0; i_6<8; i_6=i_6+1) begin:rm5to6
...
//暫存器(在每一層的micro_rotation後都要有一個reg，有load才會加載)
	genvar j_6;
	generate 
	for (j_6=0; j_6<8; j_6=j_6+1) begin:DFF_6
		always_ff@(posedge clk)begin
			if(rst)begin
				A_6q[j_6] <= 'b0;
				B_6q[j_6] <= 'b0;
...
//-----------------------micro_7-----------2^6
	signed_logic A_7q[7:0], B_7q[7:0];
	signed_logic A_rm6to7[7:0], B_rm6to7[7:0], A_7d[7:0], B_7d[7:0];
	logic p_7;
	...
	genvar i_7;
	generate 
	for (i_7=0; i_7<8; i_7=i_7+1) begin:rm6to7
...
//暫存器(在每一層的micro_rotation後都要有一個reg，有load才會加載)
	genvar j_7;
	generate 
	for (j_7=0; j_7<8; j_7=j_7+1) begin:DFF_7
		always_ff@(posedge clk)begin
			if(rst)begin
				A_7q[j_7] <= 'b0;
				B_7q[j_7] <= 'b0;
...
//乘kapa = ((2^-1+2^-3) - 2^-6  = 0.609375
	unsigned_logic kapaA13[7:0], kapaB13[7:0];
	unsigned_logic A_hat_p[7:0], B_hat_p[7:0];
	
	genvar k_1;
	generate 
	for (k_1=0; k_1<8; k_1=k_1+1) begin:kapa1
	...
	unsigned_logic kapaA13q[7:0], kapaB13q[7:0];
	always_ff@(posedge clk)begin
		if(rst)begin
	...
	genvar k_2;
	generate 
	for (k_2=0; k_2<8; k_2=k_2+1) begin:kapa2
...
//暫存器(在每一層的micro_rotation後都要有一個reg，有load才會加載)
	genvar j_9;
	generate 
	for (j_9=0; j_9<8; j_9=j_9+1) begin:DFF_9
		always_ff@(posedge clk)begin
			if(rst)begin
				A_out[j_9] <= 'b0;				
			end
	...
	genvar j_8;
	generate 
	for (j_8=0; j_8<7; j_8=j_8+1) begin:DFF_8
		always_ff@(posedge clk)begin
			if(rst)begin
				B_out[j_8] <= 'b0;
			end
