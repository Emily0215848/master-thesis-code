function [s_KB] = KB_0527(yhat, R, N, S, K, SS, M)
    ...
    elseif (2*N == 8) 
        K_layer= [1 1 1 2 2 2 2 2];%K-best-Sorted-Rii-KB_0527_1
        %K_layer= [1 1 1 4 4 4 8 8];
    else
    ...
    for layer=2*N:-1:1 
        ...
        if layer == 2*N % upper layer
           ...
           K_best(layer,:) = [S(indices(1:K1)) zeros(1, K-K1)]; % set up  upper layer nodes    
            ...
            [cands, Tcands] = deal(zeros(length(T_prev), num_symbol)); % deal func generates zeros matrices for cands, Tcands
            ...
            for path=1:num_paths % calculate M*K PEDs
                ...
                Tcands(path, :) = T_prev(path) + abs(Li - R(layer,layer) .* S(indices)); % save PEDs by asscending order %delete .^2
                cands(path, :) = S(indices); % save cands in asscending order
            end
